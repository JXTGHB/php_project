		...
		<div class="row">
			<div class="col-sm text-center" style="padding: 10px 10px">
  				<p>欲觀看課程
  					...
  					<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "程式設計一"?>">程式設計一</option>
            		<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "程式設計二"?>">程式設計二</option>
            		<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "離散數學"?>">離散數學</option>
            		<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "資料結構"?>">資料結構</option>
            		<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "資料通訊概論"?>">資料通訊概論</option>
            		<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "線性代數"?>">線性代數</option>
            		<option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "數位系統概論"?>">數位系統概論</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "電子電路"?>">電子電路</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "機率與統計"?>">機率與統計</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "資訊概論"?>">資訊概論</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "資料庫系統"?>">資料庫系統</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "作業系統概論"?>">作業系統概論</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "演算法概論"?>">演算法概論</option>
		            <option value="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo "組合語言與計算機組織"?>">組合語言與計算機組織</option>
        </select>
  				</p>
			...
			<div class="col-sm-10">
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						<?if(isset($_GET['classnm'])){?>
						<li class="page-item <?if($num_pages==1) echo 'disabled'?>"><a class="page-link" href="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo $_GET['classnm']?>&pages=<?echo $num_pages-1;?>">Previous</a></li>
						<?}?>
						<?if(isset($_GET['classnm'])==false){?>
							<li class="page-item <?if($num_pages==1) echo 'disabled'?>"><a class="page-link" href="comment_homepg_member.php?user=<?echo $_GET['user'];?>&pages=<?echo $num_pages-1;?>">Previous</a></li>
						<?}?>
						...
						<?
							for($i=1;$i<=$total_pages;$i++)
							{
								...
								$str .= "<a class='page-link' href='comment_homepg_member.php?pages={$i}'>{$i}</a></li>";
								echo $str;
							}
						?>
						<?if($total_pages>1&&(isset($_GET['classnm'])==false)){?>
                        <li class="page-item"><a class="page-link" href="comment_homepg_member.php?user=<?echo $_GET['user'];?>&pages=<?echo $num_pages+1;?>">Next</a>
                        </li>
                        <?}?>
                        <?if($total_pages>1&&isset($_GET['classnm'])){?>
                        <li class="page-item"><a class="page-link" href="comment_homepg_member.php?user=<?echo $_GET['user'];?>&classnm=<?echo $_GET['classnm']?>&pages=<?echo $num_pages+1;?>">Next</a>
                        </li>
						<?}?>
					</ul>
				</nav>
