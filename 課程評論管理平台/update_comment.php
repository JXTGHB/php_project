<?
    ...
    if(isset($_POST['action'])&&($_POST['action'] == 'update')){
        //include("connectMysql.php");
        $sql_query = "UPDATE comment SET classname=?, pressure=?, teacher=?, teach_way=?, final=? WHERE cID=?";
        $stmt = $db_link->prepare($sql_query);
        $stmt->bind_param("sssssi",$_POST['classname'],$_POST['pressure'],$_POST['teacher'],$_POST['teach_way'],$_POST['final'],$_POST['cID']);
        $stmt->execute();
        $stmt->close();
        $db_link->close();
        echo "<script>alert('修改成功');location.href='comment_homepg_admin.php?user=$nickname';</script>";
    }
    //echo $_GET['id'];
...
<div class="container border border-secondary rounded">
    ...
    <div class="row">
        <div class="col-sm">
            <h1 class="font-weight-bolder text-center">欲修改評論</h1>
        </div>
    </div>
